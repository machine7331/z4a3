...
_fishingIt = "MeleeFishingPole" in weapons player;
  ...
  s_player_removeflare4 = -1;
	...
	if (_vehicle == player and ("GdtForestPine" == surfaceType getPosATL player)) then {
		if (s_player_chopA3 < 0) then {
			s_player_chopA3 = player addAction [format["Chop Wood%1"], "z\addons\dayz_code\actions\player_chopwood.sqf"];
		};
	} else {
...
//fishing
if (_vehicle == player and _fishingIt and _isWater) then {
...
if ((_currentWeapon in Dayz_fishingItems) and !dayz_fishingInprogress and (_inVehicle and (driver _vehicle != player))) then {
